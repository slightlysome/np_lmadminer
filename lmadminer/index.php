...
*/
	...
	$oPluginAdmin  = new PluginAdmin('LMAdminer');
	...
	{
		$oPluginAdmin->start();
		echo '<p>You must be the blog super admin to use the '.htmlspecialchars($plugName, ENT_QUOTES, _CHARSET).' plugin admin area.</p>';
		$oPluginAdmin->end();
		exit;
	}
	...
	echo '<h4 class="light">Adminer page</h4>';
	echo '<div>';
	echo '<p>Manage the Nucleus CMS database with Adminer <a href="'.$pluginURL.'?username=admin&db='.htmlspecialchars($MYSQL_DATABASE, ENT_QUOTES, _CHARSET).'">here</a>.</p>';
	echo '</div></div>';
	...
	function adminer_object() 
	{
		class AdminerSoftware extends Adminer {
			...
			function credentials() 
			{
				...
				return array($MYSQL_HOST, $MYSQL_USER, $MYSQL_PASSWORD);
			}
